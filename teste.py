...
# tk.Label(root, text="Cadastro de Fabricante", font=("Arial", 12, "bold")).grid(row=7, column=0, columnspan=2, pady=10)

# tk.Label(root, text="Nome:").grid(row=8, column=0)
# entry_nome_fabricante = tk.Entry(root)
# entry_nome_fabricante.grid(row=8, column=1)

# tk.Label(root, text="CNPJ:").grid(row=9, column=0)
# entry_cnpj_fabricante = tk.Entry(root)
# entry_cnpj_fabricante.grid(row=9, column=1)

# tk.Button(root, text="Adicionar Fabricante", command=adicionar_fabricante).grid(row=10, column=0, columnspan=2, pady=5)

# # --- Movimentação de Estoque ---
# tk.Label(root, text="Movimentação de Estoque", font=("Arial", 12, "bold")).grid(row=11, column=0, columnspan=2, pady=10)

# tk.Label(root, text="Produto:").grid(row=12, column=0)
# combobox_produto = ttk.Combobox(root, state="readonly")
# combobox_produto.grid(row=12, column=1)
...
# combobox_tipo.grid(row=13, column=1)

# tk.Label(root, text="Quantidade:").grid(row=14, column=0)
# entry_quantidade_mov = tk.Entry(root)
# entry_quantidade_mov.grid(row=14, column=1)

# tk.Button(root, text="Registrar Movimentação", command=registrar_movimentacao).grid(row=15, column=0, columnspan=2, pady=5)


# # Seção de exclusão de produto
# tk.Label(root, text="Exclusão de Produto", font=("Arial", 12, "bold")).grid(row=20, column=0, columnspan=2, pady=(20, 5))

# tk.Label(root, text="ID do Produto:").grid(row=21, column=0, padx=5, pady=5)
# entry_id_produto = tk.Entry(root)
# entry_id_produto.grid(row=21, column=1, padx=5, pady=5)

# tk.Button(root, text="Excluir Produto", command=excluir_produto_por_id).grid(row=22, column=0, columnspan=2, pady=10)



# # Seção de exclusão de fabricante
# tk.Label(root, text="Exclusão de Fabricante", font=("Arial", 12, "bold")).grid(row=30, column=0, columnspan=2, pady=(20, 5))

# tk.Label(root, text="Id_fabricante:").grid(row=31, column=0, padx=5, pady=5)
# entry_id_fabricante = tk.Entry(root)
# entry_id_fabricante.grid(row=31, column=1, padx=5, pady=5)

# tk.Button(root, text="Excluir Fabricante", command=excluir_fabricante_por_id).grid(row=32, column=0, columnspan=2, pady=10)
